...
../build/gcs_fs "$BUCKET" "$MOUNT_POINT" --stat-cache-ttl=3 --debug -f 2>&1 | grep -E '\[DEBUG\]|Stat cache' &
FUSE_PID=$!
sleep 3
...
../build/gcs_fs "$BUCKET" "$MOUNT_POINT" --disable-stat-cache --debug -f 2>&1 | grep -E '\[DEBUG\]' &
FUSE_PID=$!
sleep 3
